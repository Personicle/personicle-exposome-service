...
def main(mytimer: func.TimerRequest) -> None:
    ...
    API_KEYS = os.environ.get("OPENWEATHER_KEYS").split(":")
    shuffle(API_KEYS)
    ...
    for api_key in API_KEYS:
        ...
        try:
            ...
            for i in range(ud_final.shape[0]):
                ...
                df.columns = [col.replace('current.', '')
            ...
            logging.info(
                {"msg": "successful request at {}".format(utc_timestamp)})
            break
        except Exception as e:
            logging.error({"msg": "unsuccessful request at {} for api key: {}".format(utc_timestamp, api_key),
                           "error": e})
            logging.error(traceback.format_exc())
            continue
    return
