...
def request_page():
    ...
    temp_info=pd.DataFrame(df_users['info'].values.tolist(), index=df_users.index)
    ud=df_users.copy()

    geolocator = Nominatim(user_agent="geoapiExercises")
    temp_info['latitude']=(temp_info['zipcode']+' '+temp_info['country']).swifter.apply(lambda x:geolocator.geocode(x).latitude)
    temp_info['longitude']=(temp_info['zipcode']+' '+temp_info['country']).swifter.apply(lambda x:geolocator.geocode(x).longitude)


    ud_final=pd.concat([ud[['email']],temp_info[['country','zipcode','latitude','longitude']]],axis=1)#.to_csv('test.csv')
    
    
    lat = np.float(ud_final[ud_final.email==user].iloc[:,ud_final.columns.get_loc('latitude')])
    lon = np.float(ud_final[ud_final.email==user].iloc[:,ud_final.columns.get_loc('longitude')])
    
  
    
    
    url = "https://api.openweathermap.org/data/2.5/onecall?lat=%s&lon=%s&appid=%s&units=metric" % (lat, lon, api_key_weather)
    ...
    url1=  "http://api.openweathermap.org/data/2.5/air_pollution/forecast?lat=%s&lon=%s&appid=%s" % (lat, lon, api_key_pol)
    ...
    df_comb['streamName']='com.personicle.individual.datastreams.exposome.'+df_comb['streamName']
    ...
    try:
        ...
        logging.info("Total data points added for source {}: {}".format(datastream_producer(res[i]), len(datastream_producer(res[i]))))
